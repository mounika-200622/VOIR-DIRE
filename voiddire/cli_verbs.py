...
def cmd_init(a) -> int:
    ...
    if not found:
        ...
        if state == "not-a-repo":
            ...
            print("  teach it directly instead:  voiddire rule \"src/*.py needs tests/\"")
            ...
            print("    voiddire rule \"src/*.py needs tests/\"")
        else:
    ...
    print()
    print(f"  voiddire confirm {ids}    to make them binding")
    print()
    return 0
...
def cmd_opencode(a) -> int:
    ...
    print("    voiddire serve")
    print()
    if a.mode != "enforce":
        print(f"  mode {a.mode}: {MODES[a.mode]}")
        print(f"    VOIDDIRE_MODE={a.mode} opencode")
    else:
