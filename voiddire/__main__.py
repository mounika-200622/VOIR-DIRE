...
def cmd_bench(a) -> int:
    ...
    seeds = tuple(int(x) for x in a.seeds.split(",") if x.strip())
    if "B" in arms and not getattr(a, "live", False):
        print("arm B is only meaningful with --live: its mechanism is whether a MODEL")
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="voiddire")
    ...
    m.set_defaults(fn=lambda a: __import__("voiddire.demo", fromlist=["main"]).main())
    ...
    v.set_defaults(fn=lambda a: __import__("voiddire.serve", fromlist=["main"]).main(a.port))
    ...
    rp.set_defaults(fn=lambda a: __import__("voiddire.replay", fromlist=["main"]).main(a.repo))
    ...
    lv.set_defaults(fn=lambda a: __import__("voiddire.live", fromlist=["main"]).main(
    ...
    w.set_defaults(fn=lambda a: __import__("voiddire.board", fromlist=["main"]).main(a.port, a.ledger))
    ...
    wt.set_defaults(fn=lambda a: __import__("voiddire.watch", fromlist=["main"]).main(
