...
def _pick_ledger() -> Path | None:
    for c in (ROOT / ".bench" / "C" / "1" / "ledger.db",
              ROOT / ".live" / "trial" / ".voiddire" / "ledger.db",
              ROOT / ".demo" / "ledger.db"):
        if c.is_file():
            return c
    return None
...
NEEDS = [
    ("data.json",       "python -m voiddire dump"),
    ("report.json",     "python -m voiddire bench --arms A,C --seeds 1,2,3,4,5"),
    ("compliance.json", "python -m voiddire bench --compliance"),
    ("replay.json",     "python -m voiddire replay"),
]
