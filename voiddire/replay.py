...
The desk is a live view of `voiddire watch`. Nobody opening the static site
...
def record(repo: Path = REPO, ledger: Path = LEDGER,
    ...
    if not repo.is_dir() or not ledger.is_file():
        raise SystemExit(
            ...
            "  run: python -m voiddire bench --arms A,C --seeds 1")
...
def _restore(repo: Path, before: dict) -> None:
    ...
    for p in list(repo.rglob("*")):
        ...
        if rel not in before and not rel.startswith(".voiddire/"):
            p.unlink()
