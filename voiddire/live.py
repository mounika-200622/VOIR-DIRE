...
# use all of it.
WORKROOT = Path(
    __import__("os").environ.get("VOIDDIRE_LIVE_HOME", Path.home() / ".voiddire-live"))
...
def _reset_ledger(repo: Path) -> None:
    ...
    pristine = repo / ".voiddire" / "ledger.seed.db"
    live = repo / ".voiddire" / "ledger.db"
    if pristine.is_file():
...
def setup(seed: Path, repo: Path) -> dict:
    ...
    led = Ledger(repo / ".voiddire" / "ledger.db")
    taught = teach(led, repo)
    led.close()
    restore(seed, repo)
    shutil.copy2(repo / ".voiddire" / "ledger.db", repo / ".voiddire" / "ledger.seed.db")
    ...
    shutil.copy2(ROOT / "plugin" / "voiddire.ts", plugins / "voiddire.ts")
...
def _repo_untouched() -> None:
    ...
    still edited seeds/, voiddire/ and wrote a migrations/ directory at the
    ...
    if dirty:
        raise RuntimeError(
            "the agent modified the void dire repository during a trial:"
...
def _armed(repo: Path, led: Ledger, port: int = 4000) -> None:
    ...
    plugin = repo / ".opencode" / "plugins" / "voiddire.ts"
...
def trial(repo: Path, seed: Path, led: Ledger, task: str, model: str = MODEL,
    ...
    proc = shell.run_env(cmd, repo, {"VOIDDIRE_MODE": mode}, timeout=timeout)
    ...
    return {
        ...
        "halted": "BLOCKED BY VOID DIRE" in said,
        "touched": ch.touched,
