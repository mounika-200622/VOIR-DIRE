// VOID DIRE — an opencode plugin that refuses.
//
...
//
// Every decision is made by `python -m voiddire serve`. This file is plumbing:
// bounded timeouts, fail-open, and never a reason the agent stalls.

const URL_BASE = process.env.VOIDDIRE_URL ?? "http://127.0.0.1:4000"
const TIMEOUT_MS = Number(process.env.VOIDDIRE_TIMEOUT_MS ?? 900)
const CHECK = process.env.VOIDDIRE_CHECK ?? "python oracle.py"
...
const MODE = (process.env.VOIDDIRE_MODE ?? "enforce").toLowerCase()
...
export default { id: "voiddire", server: VoidDire }
export { VoidDire }
