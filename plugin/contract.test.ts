...
//
// Requires `python -m voiddire serve` to be running.

import { VoidDire } from "./voiddire.ts"
...
const hooks: any = await VoidDire({ directory: repo, worktree: repo })
...
check("the card names the precedent", !!threw?.message?.includes("Holding No"))
...
for (const mode of ["off", "advise", "enforce"]) {
  process.env.VOIDDIRE_MODE = mode
  const mod: any = await import(`./voiddire.ts?m=${mode}`)
  const h: any = await mod.VoidDire({ directory: repo, worktree: repo })
