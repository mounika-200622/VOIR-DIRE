...
The AI is not allowed to edit `.voiddire/`, the git hooks, the plugin file, the
...
Mutes a rule. Keeps the case that created it. `voiddire on 4` brings it back.
...
```bash
voiddire board      # the office  -> http://127.0.0.1:8850
voiddire demo       # the race    -> http://127.0.0.1:8900
```
...
- *"What if I disagree with a rule?"* → `voiddire off 4`. Mutes it, keeps the
...
- The 25 rules that ship on day one, and **the one that stops the agent
...
- **Never open the race from the board.** It lives on **8900** under
  `voiddire demo`. The copy at 8850 has no apps behind it and the panes go
  blank.
