...
def similar(led: Ledger, ward: str, task: str, k: int = 5,
            statuses: tuple[str, ...] = ("binding", "persuasive")) -> list[dict]:
    q = _vec(task)
    scored = []
    for h in led.holdings(ward=ward):
...
def everything(led: Ledger, ward: str, task: str, cap: int = 40) -> list[dict]:
    ...
    """
    q = _vec(task)
    rows = [h for h in led.holdings(ward=ward)
...
def briefing(led: Ledger, ward: str, task: str, k: int = 5, cap: int = 12) -> str:
    ...
    patient model" against "models/*.py" at exactly zero. So a ward
    taught its rules directly, which is every ward on day one, got
    nothing.
    ...
    relevance.
    """
    binding = [h for h in led.holdings(ward=ward) if h["status"] == "binding"]
    advisory = [r for r in similar(led, ward, task, k=k, statuses=("persuasive",))]
    if not binding and not advisory:
        return ""

    lines: list[str] = []
    if binding:
        lines.append("This ward will STOP a change that breaks these. "
