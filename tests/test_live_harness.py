...
from voiddire import gate, live
...
def test_setup_leaves_the_agents_own_configuration_in_place(workspace):
    ...
    repo, _ = workspace
    assert (repo / ".opencode" / "plugins" / "voiddire.ts").is_file()
...
def test_armed_refuses_a_workspace_with_no_plugin(workspace):
    repo, _ = workspace
    (repo / ".opencode" / "plugins" / "voiddire.ts").unlink()
    led = Ledger(repo / ".voiddire" / "ledger.db")
...
def test_the_trigger_path_comes_from_the_rule_itself(workspace):
    _, taught = workspace
    from voiddire.db import Ledger as L
    repo, _ = workspace
    led = L(repo / ".voiddire" / "ledger.db")
